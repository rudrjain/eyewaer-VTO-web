//const {Controller, UI} = window.MEDIAPIPEAR.FACE;
...
AFRAME.registerSystem('mediapipear-face-system', {
  container: null,
  video: null,
  shouldFaceUser: true,
  lastHasFace: false,
...
AFRAME.registerComponent('mediapipear-face', {
  dependencies: ['mediapipear-face-system'],
  ...
  init: function() {
    const arSystem = this.el.sceneEl.systems['mediapipear-face-system'];

    if (this.data.faceOccluder) {
      const faceOccluderMeshEntity = document.createElement('a-entity');
      faceOccluderMeshEntity.setAttribute("mediapipear-face-default-face-occluder", true);
...
AFRAME.registerComponent('mediapipear-face-target', {
  dependencies: ['mediapipear-face-system'],
...
AFRAME.registerComponent('mediapipear-face-occluder', {
  init: function() {
    const root = this.el.object3D;
...
AFRAME.registerComponent('mediapipear-face-default-face-occluder', {
  init: function() {
    const arSystem = this.el.sceneEl.systems['mediapipear-face-system'];
...
*/
/* if(needsDOMRefresh){
  console.log("mediapipear-face-aframe::Refreshing DOM...")
